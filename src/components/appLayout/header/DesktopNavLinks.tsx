                ...
                {navigationLinks.map(({ href, label }: NavigationLink) => (
                    <NavigationMenuItem key={href} >
                        <NavigationMenuLink asChild className="text-lg">
                            <Link
                                href={href}
                                className={`font-semibold capitalize ${pathname == href ? 'text-[#C05C41] hover:text-[#7B4B3A]' : 'text-[#7B4B3A] hover:text-[#C05C41]'} ${(unreadMessages && label == 'conversations') && 'text-red-500'}`}
                            >
                                {label}
                            </Link>
                        </NavigationMenuLink>
